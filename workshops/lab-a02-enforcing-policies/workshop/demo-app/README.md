# Chainloop Demo App — A02

A Next.js web application used as the sample project for the **Enforcing Policies and Governing Compliance** workshop (Lab A02). This is the **A01 end-state** repository: the GitHub Actions workflow already has Chainloop attestation integrated. Learners add SBOM generation and policy enforcement during the workshop.

## Workflow (A02 starting state)

```
attestation-init → build-push → attestation-push
```

Required GitHub Actions secrets (set automatically by the workshop session setup):
- `CHAINLOOP_TOKEN` — authenticates the Chainloop CLI
- `CHAINLOOP_WORKFLOW` — the workflow name scoped to the learner's session

> Based on [educates-oauth-simple-frontend](https://github.com/jorgemoralespou/educates-oauth-simple-frontend).
