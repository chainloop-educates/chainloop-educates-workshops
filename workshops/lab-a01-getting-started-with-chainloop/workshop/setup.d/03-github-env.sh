# #!/bin/bash
# # Set up the GitHub repository for this workshop session.
# #
# # Uses the admin PAT (from the 'github-pats' K8s secret) to create a fresh
# # repository in the chainloop-educates org, then pushes the bundled demo app
# # source from workshop/demo-app/.
# #
...
# # Configure gh CLI with the admin PAT for setup operations
# export GH_TOKEN="${GITHUB_ADMIN_PAT}"

# # Delete any existing repo from a previous session (ignore error if it doesn't exist)
...
# # ── Push bundled demo app source ──────────────────────────────────────────────
# echo "Populating repository from bundled source..."
# WORK_DIR=$(mktemp -d)
...
# git add -A
# git commit -m "Initial commit: Chainloop demo app (A01 starting state)"
